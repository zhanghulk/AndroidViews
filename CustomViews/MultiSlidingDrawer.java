...
public class MultiSlidingDrawer extends ViewGroup {
	...
	public static final String	LOG_TAG	= "MultiSlidingDrawer";
	public static boolean debugMode	= false;
	
	public static final int				DIRECTION_RTL		= 0;
	public static final int				DIRECTION_BTT		= 1;
	public static final int				DIRECTION_LTR		= 2;
	public static final int				DIRECTION_TTB		= 3;
	...
	private static final int			ANIMATION_FRAME_DURATION	= 1000 / 60;//animation interval time
	...
	private float                       mHandleOffsetPercentage;
	private static int			        mAnimateFrameDuration;
	private static int			        mAnimateVelocityUnits;
	private int direction = DIRECTION_BTT;
	
	private OnDrawerOpenListener		mOnDrawerOpenListener;
	private OnDrawerCloseListener		mOnDrawerCloseListener;
	private OnDrawerScrollListener	    mOnDrawerScrollListener;
	private OnDrawerStateListener	    mOnDrawerStateListener;
	private Drawable mHandleOpenedBg;
	private Drawable mHandleClosedBg;
	 ...
	 */
	public static interface OnDrawerStateListener {
		/**
		 * Invoked when the panel becomes fully closed.
		 */
		public void onDraweClosed(MultiSlidingDrawer drawer);

		/**
		 * Invoked when the panel becomes fully opened.
		 */
		public void onDraweOpened(MultiSlidingDrawer drawer);
	}
	...
	public MultiSlidingDrawer( Context context, AttributeSet attrs, int defStyle )
	{
		...
		direction = a.getInt( R.styleable.MultiSlidingDrawer_direction, DIRECTION_BTT );
		mVertical = ( direction == DIRECTION_BTT || direction == DIRECTION_TTB );
		mBottomOffset = (int)a.getDimension( R.styleable.MultiSlidingDrawer_bottomOffset, 0.0f );
		...
		mInvert = ( direction == DIRECTION_RTL || direction == DIRECTION_LTR );
		mHandleOpenedBg = a.getDrawable(R.styleable.MultiSlidingDrawer_handleOpenedBg);
		mHandleClosedBg = a.getDrawable(R.styleable.MultiSlidingDrawer_handleClosedBg);
		mAnimateFrameDuration = (int)a.getInt( R.styleable.MultiSlidingDrawer_animateFrameDuration, ANIMATION_FRAME_DURATION );
		mAnimateVelocityUnits = (int)a.getInt( R.styleable.MultiSlidingDrawer_animateVelocityUnits, VELOCITY_UNITS );
		mHandleOffset = (int)a.getDimension( R.styleable.MultiSlidingDrawer_handleOffset, 0.0f );
		mHandleOffsetPercentage = (float)a.getFloat( R.styleable.MultiSlidingDrawer_handleOffsetPercentage, 1.0f );
		
		int handleId = a.getResourceId( R.styleable.MultiSlidingDrawer_handle, 0 );
		if ( handleId == 0 ) { throw new IllegalArgumentException( "The handle attribute is required and must refer "
				+ "to a valid child." ); }
		
		int contentId = a.getResourceId( R.styleable.MultiSlidingDrawer_content, 0 );
		...
		mVelocityUnits = (int)( mAnimateVelocityUnits * density + 0.5f );
	...
	public void setDirection(int orientation) {
		...
		mInvert = ( orientation == DIRECTION_RTL || orientation == DIRECTION_LTR );
	...
	protected void onFinishInflate()
	{
		...
		if (mHandleClosedBg != null) {
			mHandle.setBackgroundDrawable(mHandleClosedBg);
		}
	...
	protected void onMeasure( int widthMeasureSpec, int heightMeasureSpec )
	{
		...
		if ( widthSpecMode == MeasureSpec.UNSPECIFIED || heightSpecMode == MeasureSpec.UNSPECIFIED ) { 
			Log.e(LOG_TAG, "widthMeasureSpec= " + widthMeasureSpec + ", heightMeasureSpec= " + heightMeasureSpec);
			Log.e(LOG_TAG, "widthSpecMode= " + widthSpecMode + ", heightSpecMode= " + heightSpecMode);
			Log.e(LOG_TAG, "widthSpecSize= " + widthSpecSize + ", heightSpecSize= " + heightSpecSize);
	...
	protected void onLayout( boolean changed, int l, int t, int r, int b )
	{
		...
		if ( mVertical ) {
			if(mHandleOffset <= 0) {
				mHandleOffset = width - handleWidth;
			}
			if(isValidHandleOffsetPercentage()) {
				handleLeft = caculateOffset(mHandleOffset, mHandleOffsetPercentage);
			} else {
				handleLeft = mHandleOffset;
			}
			...
			if(mHandleOffset <= 0) {
				mHandleOffset = height - handleHeight;
			} else {
				log("mHandleOffset == " + mHandleOffset);
			}
			if(isValidHandleOffsetPercentage()) {
				log("mHandleOffsetPercentage == " + mHandleOffsetPercentage);
				handleTop = caculateOffset(mHandleOffset, mHandleOffsetPercentage);
			} else {
				handleTop = mHandleOffset;
			}
			if( mInvert ) {
		...
		log( "ONLAYOUT: " + changed +",mVertical: " + mVertical +",mInvert: " + mInvert 
				+ ",handleTop: " + handleTop + ",handleLeft: " + handleLeft 
				+ ",handleWidth: " + handleWidth + ",handleHeight: " + handleHeight
				+ ",mTopOffset: " + mTopOffset + ",mBottomOffset: " + mBottomOffset );
		handle.layout( handleLeft, handleTop, handleLeft + handleWidth, handleTop + handleHeight );
		mHandleHeight = handle.getHeight();
		mHandleWidth = handle.getWidth();
	}
	
	private int caculateOffset(int maxOffset, float percentage) {
		int offSet = Math.round(maxOffset * percentage);
		return offSet;
	}
	
	public boolean isValidHandleOffsetPercentage() {
		return isValidPercentage(mHandleOffsetPercentage);
	}
	...
	public void setHandleOffsetPercentage(float offsetPercentage) {
		mHandleOffsetPercentage = offsetPercentage;
		invalidate();
		requestLayout();
	}
			...
			switch ( action ) {
				...
				case MotionEvent.ACTION_CANCEL: {
					...
					if ( Math.abs( velocity ) < mMaximumTapVelocity ) {
						...
						log( "ACTION_UP: " + "c1: " + c1 + ", c2: " + c2 + ", c3: " + c3 + ", c4: " + c4 );
	...
	private void performFling( int position, float velocity, boolean always )
	{
		...
		if ( mExpanded ) 
		{
			...
			log( "position: " + position + ", velocity: " + velocity + ", mMaximumMajorVelocity: " + mMaximumMajorVelocity );
			...
			log( "EXPANDED. c1: " + c1 + ", c2: " + c2 + ", c3: " + c3 );
			if ( always || ( c1 || ( c2 && c3 ) ) ) {
			...
			log( "COLLAPSED. position: " + position + ", velocity: " + velocity + ", mMaximumMajorVelocity: " + mMaximumMajorVelocity );
			log( "COLLAPSED. always: " + always + ", c1: " + c1 + ", c2: " + c2 + ", c3: " + c3 );
		...
		mCurrentAnimationTime = now + mAnimateFrameDuration;
		mAnimating = true;
	...
	private void postStateChangeAction() {
		// invoke listener if any
		final boolean isOpen = mExpanded;
		if (isOpen && mHandleOpenedBg != null) {
			mHandle.setBackgroundDrawable(mHandleOpenedBg);
		} else if (!isOpen && mHandleClosedBg != null) {
			mHandle.setBackgroundDrawable(mHandleClosedBg);
		} else {
			log("handle drawable is null !! ");
		}
		if (mOnDrawerStateListener != null) {
			if (!isOpen) {
				mOnDrawerStateListener.onDraweClosed(MultiSlidingDrawer.this);
			} else {
				mOnDrawerStateListener.onDraweOpened(MultiSlidingDrawer.this);
			}
		}
	}
	...
	private void prepareContent()
	{
		...
		final View content = mContent;
		boolean isRequested = content.isLayoutRequested();
		log( "isRequested:" + isRequested + ",mVertical:" + mVertical + ", mInvert:" + mInvert + ",mTopOffset:" + mTopOffset);
		if ( isRequested ) {
			...
			if ( mVertical ) {
				...
				log( "handleHeight:" + handleHeight + ",height:" + height);
				if ( mInvert ) 
					content.layout( 0, mTopOffset, content.getMeasuredWidth(), mTopOffset + content.getMeasuredHeight() );
				else 
					content.layout( 0, mTopOffset + handleHeight, content.getMeasuredWidth(), mTopOffset + handleHeight + content.getMeasuredHeight() );
				...
				log( "handleWidth:" + handleWidth + ",width:" + width);
		...
		if ( mAnimating ) {
			...
			if( mInvert )
			{
				if ( mAnimationPosition < mTopOffset ) {
					...
					moveHandle( (int)mAnimationPosition );
					mCurrentAnimationTime += mAnimateFrameDuration;
					mHandler.sendMessageAtTime( mHandler.obtainMessage( MSG_ANIMATE ), mCurrentAnimationTime );
				}				
			} else {
	...
	/**
	 * set drawer open state: open it if isOpen, or close drawer
	 * @param isOpen 
	 * @param isAnimate
	 */
	public void  setOpen(boolean isOpen, boolean isAnimate) {
		//^ ： XOR operation(yihuoyunsuan)
		if(isOpened() ^ isOpen) {
			if(isAnimate) {
				if(isOpen) {
					animateOpen();
			...
			Log.i(LOG_TAG, "The same action isOpen: " + isOpen);
		}
	}
	...
	/**
	 *  Sets the listener that receives a notification when the drawer is opened and closed.
	 * @param onDrawerStateListener
	 */
	public void setOnDrawerStateListener( OnDrawerStateListener onDrawerStateListener )
	{
		mOnDrawerStateListener = onDrawerStateListener;
	}
	...
	public Drawable getHandleOpenedBg() {
		return mHandleOpenedBg;
	}

	public void setHandleOpenedBg(Drawable handleOpenedBg) {
		this.mHandleOpenedBg = handleOpenedBg;
	}

	public Drawable getHandleClosedBg() {
		return mHandleClosedBg;
	}

	public void setHandleClosedBg(Drawable handleClosedBg) {
		this.mHandleClosedBg = handleClosedBg;
	}
	
	public void setHandleOpenedBgResource(int resId) {
		this.mHandleOpenedBg = getContext().getResources().getDrawable(resId);
	}

	public void setHandleClosedBgResource(int resId) {
		this.mHandleClosedBg = getContext().getResources().getDrawable(resId);
	}
	...
	public void setmAllowSingleTap(boolean allowSingleTap) {
	...
	public static int getAnimateFrameDuration() {
		return mAnimateFrameDuration;
	}

	public static void setAnimateFrameDuration(int duration) {
		mAnimateFrameDuration = duration;
	}
	...
	public static void setDebugMode(boolean debugMode) {
		MultiSlidingDrawer.debugMode = debugMode;
	}
}
